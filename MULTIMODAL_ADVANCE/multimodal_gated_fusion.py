...
os.environ["CUDA_VISIBLE_DEVICES"] = "4" # Set the desired GPU device
...
CHECKPOINT_PATH = "best_multimodal_clip_rn18_gated_fusion.pt" # Updated checkpoint name
...
# ==================== MODIFIED MULTIMODAL MODEL (GATED FUSION) ====================

class GatedFusion(nn.Module):
    """
    A Gated Fusion Network where the Vision feature generates a gate
    to modulate the Audio feature before combination.
    """
    def __init__(self, audio_dim, vision_dim, gate_dim=512, hidden_dim=512, dropout_rate=0.5):
        super().__init__()
        # 1. Audio Projection (to align dimensions with the gate and final feature)
        self.audio_proj = nn.Sequential(
            nn.Linear(audio_dim, gate_dim),  # 512 -> 512
            nn.ReLU(inplace=True),
            nn.Dropout(p=dropout_rate)
        )

        # 2. Vision Gate Generator (computes the scaling factor for audio)
        self.vision_gate = nn.Sequential(
            nn.Linear(vision_dim, gate_dim),  # 512 -> 512
            nn.Sigmoid()  # Output values between 0 and 1
        )

        # 3. Final Head (applied after the gated combination)
        self.final_head = nn.Sequential(
            nn.Linear(gate_dim, hidden_dim),  # 512 -> 512 (can be removed if no further non-linearity is desired)
            nn.ReLU(inplace=True),
            nn.Dropout(p=dropout_rate),
        )
    ...
    def forward(self, audio_features, vision_features):
        # 1. Project Audio
        a_proj = self.audio_proj(audio_features)  # [B, 512]

        # 2. Generate Gate from Vision
        g_v = self.vision_gate(vision_features)  # [B, 512]

        # 3. Gated Fusion: Fused = (Vision Gate * Projected Audio) + Vision Feature
        fused_intermediate = (g_v * a_proj) + vision_features  # [B, 512]

        # 4. Final Head
        return self.final_head(fused_intermediate)


class MultimodalClassifier_Gated(nn.Module):
    def __init__(self, audio_extractor, vision_extractor, num_classes, dropout_rate=0.5):
        ...
        # Vision feature is 1024. Downscale it to 512 for the Gated Fusion layer.
        ...
        # Gated Fusion Layer: Takes both 512-D features and outputs a 512-D fused feature
        self.gated_fusion_layer = GatedFusion(
            audio_dim=512, 
            vision_dim=512, 
            gate_dim=512, 
            hidden_dim=512, 
        ...
        # Final Classification Head (takes the 512-D output from GatedFusion)
        ...
        trainable_params.extend(list(self.gated_fusion_layer.parameters()))
        trainable_params.extend(list(self.classifier_head.parameters()))

        print(f"Total Trainable Parameters (SE+CBAM+GatedFusion+Head): {sum(p.numel() for p in trainable_params):,}")


    def forward(self, audio, vision):
        # 1. Extract and Refine Modality-Specific Features
        audio_features = self.audio_extractor(audio)  # [B, 512]
        vision_features = self.vision_extractor(vision)  # [B, 1024]
        
        # 2. Project and Prepare for Gated Fusion
        audio_features_prep = self.audio_head(audio_features)  # [B, 512]
        vision_features_prep = self.vision_projection(vision_features)  # [B, 512]
        
        # 3. Gated Fusion
        fused_features = self.gated_fusion_layer(audio_features_prep, vision_features_prep)  # [B, 512]
...
if __name__ == '__main__':
    ...
    # *** SWITCHED TO NEW GATED CLASSIFIER ***
    model = MultimodalClassifier_Gated(audio_extractor, vision_extractor, NUM_CLASSES, dropout_rate=DROPOUT).to(DEVICE)

    # SpecAugment for audio only
    spec_augmenter = SpecAugment().to(DEVICE)

    # Only optimize the trainable parts (SEBlock, CBAM, GatedFusion/Head)
    ...
    # 4. Training Loop
    print("\n--- Starting Multimodal Training with Gated Fusion ---")
