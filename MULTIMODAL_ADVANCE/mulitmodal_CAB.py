...
CHECKPOINT_PATH = "best_multimodal_clip_rn18_cab_fusion.pt" # Updated checkpoint name

# --- HYPERPARAMETERS ---
BATCH_SIZE = 32 # Consider reducing this if OOM errors persist
EPOCHS = 100
...
if DEVICE == "cuda":
    torch.cuda.empty_cache() # Attempt to clear VRAM before loading large model
try:
    clip_model, preprocess_val = clip.load(CLIP_BACKBONE, device=DEVICE, jit=False)
except RuntimeError as e:
    print(f"FATAL ERROR during CLIP loading: {e}")
    print("TIP: Try reducing BATCH_SIZE or changing CLIP_BACKBONE to 'RN34' or 'ViT-B/32'.")
    exit()
...
# ==================== CAB MODALITY ATTENTION GENERATOR ====================

class ModalityAttentionGenerator(nn.Module):
    """
    Auxiliary head that takes a pooled feature (e.g., 512D) and generates a mask (512D)
    by passing it through a linear layer and a Sigmoid activation.
    This serves as the F^T(X) -> GAP -> Sigmoid part of the CAB mechanism.
    """
    def __init__(self, feature_dim, dropout_rate=0.5):
        super().__init__()
        # Linear layer and dropout to introduce trainable parameters and regularization
        self.mask_generator = nn.Sequential(
            nn.Linear(feature_dim, feature_dim), 
            nn.Dropout(p=dropout_rate),
            nn.Sigmoid() # The mask M, with values between 0 and 1
        )
...
# ==================== MODIFIED MULTIMODAL MODEL (CAB FUSION) ====================
...
class MultimodalClassifier_CAB(nn.Module):
    def __init__(self, audio_extractor, vision_extractor, num_classes, dropout_rate=0.5):
        ...
        # --- Pre-Fusion Projection/Normalization ---
        
        # Vision feature is 1024. Downscale it to 512 for fusion.
        ...
        # Audio feature is 512 (RN18 output) - just normalization and dropout
        self.audio_head = nn.Sequential(
            nn.BatchNorm1d(audio_extractor.output_dim),  # 512
            nn.Dropout(p=dropout_rate)
        )

        # --- Cross-Attention Blocks (CAB) Setup ---
        FEATURE_DIM = 512
        
        # 1. Vision Mask Generator (creates mask M_V for Audio)
        self.vision_mask_generator = ModalityAttentionGenerator(FEATURE_DIM, dropout_rate)
        
        # 2. Audio Mask Generator (creates mask M_A for Vision)
        self.audio_mask_generator = ModalityAttentionGenerator(FEATURE_DIM, dropout_rate)
        
        # --- Final Classification Head ---
        FUSED_DIM = FEATURE_DIM * 2  # 512 + 512 = 1024
        HIDDEN_DIM = FUSED_DIM // 2  # 512
        ...
        # CAB specific trainable parameters:
        trainable_params.extend(list(self.vision_mask_generator.parameters()))
        trainable_params.extend(list(self.audio_mask_generator.parameters()))
        trainable_params.extend(list(self.classifier_head.parameters()))

        print(f"Total Trainable Parameters (SE+CBAM+CAB+Head): {sum(p.numel() for p in trainable_params):,}")


    def forward(self, audio, vision):
        # 1. Extract and Refine Modality-Specific Features (A_raw, V_raw)
        audio_features_raw = self.audio_extractor(audio)  # [B, 512]
        vision_features_raw = self.vision_extractor(vision)  # [B, 1024]
        
        # 2. Project and Prepare for CAB (A_proj, V_proj)
        A_proj = self.audio_head(audio_features_raw)  # [B, 512]
        V_proj = self.vision_projection(vision_features_raw)  # [B, 512]
        
        # 3. Generate Cross-Attention Masks (M_A, M_V)
        # M_V is derived from Vision (V_proj) and will modulate Audio (A_proj)
        M_V = self.vision_mask_generator(V_proj) 
        # M_A is derived from Audio (A_proj) and will modulate Vision (V_proj)
        M_A = self.audio_mask_generator(A_proj) 
        
        # 4. Symmetric Cross-Refinement (A', V') - X' = X + (X * M)
        # Refine Audio using the Vision Mask (M_V)
        A_refined = A_proj + (A_proj * M_V) 
        # Refine Vision using the Audio Mask (M_A)
        V_refined = V_proj + (V_proj * M_A)
        
        # 5. Final Concatenation/Fusion
        fused_features = torch.cat([A_refined, V_refined], dim=1)  # [B, 1024]
...
if __name__ == '__main__':
    ...
    # *** SWITCHED TO NEW CAB CLASSIFIER ***
    model = MultimodalClassifier_CAB(audio_extractor, vision_extractor, NUM_CLASSES, dropout_rate=DROPOUT).to(DEVICE)

    # SpecAugment for audio only
    spec_augmenter = SpecAugment().to(DEVICE)

    # Only optimize the trainable parts (SEBlock, CBAM, CAB/Head)
    ...
    # 4. Training Loop
    print("\n--- Starting Multimodal Training with Cross-Attention Block (CAB) Fusion ---")
