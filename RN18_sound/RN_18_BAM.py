...
CHECKPOINT_PATH = "best_rn18_bam_model_80_20.pt"
...
class SpatialGate(nn.Module):
    def __init__(self, gate_channels, reduction_ratio=16, dilation_conv_num=2, dilation_val=4):
        ...
        self.gate_channels = gate_channels
        self.corn1x1 = nn.Conv2d(gate_channels, gate_channels // reduction_ratio, kernel_size=1)
        ...
        self.corn2x2 = nn.Conv2d(gate_channels // reduction_ratio, 1, kernel_size=1)
    def forward(self, x):
        x = self.corn1x1(x)
        for i in range(len(self.conv_list)):
            x = self.conv_list[i](x)
        return self.corn2x2(x).expand_as(x)
