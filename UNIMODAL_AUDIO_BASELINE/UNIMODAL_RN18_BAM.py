...
# Checkpoint name changed to reflect BAM L4-only usage
CHECKPOINT_PATH = "best_rn18_advance_audio_bam_l4only.pth" 
...
PATIENCE = 15
MIN_DELTA = 5e-4
BAM_REDUCTION = 16 # Reduction ratio for Channel Attention
...
class ChannelAttentionModule(nn.Module):
    """Channel Attention Module (Part of BAM)"""
    def __init__(self, channels, reduction=16):
        super(ChannelAttentionModule, self).__init__()
        # Global Average Pooling for Squeeze operation
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        
        # Multi-Layer Perceptron (MLP) - Only using Linear layers for 1D vector input
        self.mlp = nn.Sequential(
            # Input shape: [B, C]. Output shape: [B, C // R]
            nn.Linear(channels, channels // reduction, bias=False), 
            nn.BatchNorm1d(channels // reduction),
            nn.ReLU(inplace=True),
            # Input shape: [B, C // R]. Output shape: [B, C]
    ...
    def forward(self, x):
        ...
        y = self.avg_pool(x).squeeze(-1).squeeze(-1) # Output [B, C]
        
        # 2. Excitation: MLP -> [B, C]
        y = self.mlp(y)
        
        # 3. Reshape to [B, C, 1, 1] for broadcasting and addition
        return y.unsqueeze(-1).unsqueeze(-1)


class SpatialAttentionModule(nn.Module):
    # ... (This module remains unchanged from your original script)
    """Spatial Attention Module (Part of BAM)"""
    def __init__(self, channels, reduction=16, dilation_rate=4):
        ...
        self.conv2 = nn.Sequential(
            nn.Conv2d(channels // reduction, channels // reduction, kernel_size=3, padding=dilation_rate, dilation=dilation_rate),
            nn.BatchNorm2d(channels // reduction),
            nn.ReLU(inplace=True)
        )
        
        # Final 1x1 conv to output 1 channel for spatial attention map
        self.conv3 = nn.Conv2d(channels // reduction, 1, kernel_size=1)
...
class BAM(nn.Module):
    ...
    def __init__(self, channels, reduction=16):
        ...
        self.ca = ChannelAttentionModule(channels, reduction=reduction)
        self.sa = SpatialAttentionModule(channels, reduction=reduction)
        self.sigmoid = nn.Sigmoid()

    def forward(self, x):
        # Calculate attention maps in parallel
        channel_map = self.ca(x)
        spatial_map = self.sa(x)
        
        # Combine maps via element-wise addition (broadcasted)
        attention_map = channel_map + spatial_map
        
        # Apply the final attention mask to the input
        return x * (1 + self.sigmoid(attention_map))

# ==================== ResNet18 with BAM (Layer 4 Only) ====================

class ResNet18_BAM_L4Only(nn.Module):
    def __init__(self, num_classes, reduction=BAM_REDUCTION):
        super().__init__()
        ...
        # Instantiate **ONLY** BAM after stage 4 (Channel count for layer4 is 512)
        self.bam = BAM(512, reduction=reduction)
    ...
    def forward(self, x):
        ...
        x = self.layer4(x)
        x = self.bam(x) # <--- BAM only after layer4
...
if __name__ == '__main__':
    ...
    # Model with BAM (Layer 4 Only)
    model = ResNet18_BAM_L4Only(NUM_CLASSES, reduction=BAM_REDUCTION).to(DEVICE)
