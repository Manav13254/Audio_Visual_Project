...
CHECKPOINT_PATH = "best_image_clip_senet_checkpoint.pt"
...
class CLIP_RN50_SE_Head(nn.Module):
    def __init__(self, clip_model, num_classes, dropout_rate=0.5):
        ...
        self.se2 = SELayer(self.clip_visual.layer2[-1].conv3.out_channels)
        self.se3 = SELayer(self.clip_visual.layer3[-1].conv3.out_channels)
        self.se4 = SELayer(self.clip_visual.layer4[-1].conv3.out_channels)
    ...
    def forward(self, images):
        ...
        x = v.layer1(x)
        x = v.layer2(x); x = self.se2(x)
        x = v.layer3(x); x = self.se3(x)
        x = v.layer4(x); x = self.se4(x)
        x = v.attnpool(x)
...
if __name__ == '__main__':
    ...
    mlflow.set_experiment("image_clip_senet_tuning")
    with mlflow.start_run():
        ...
        if os.path.exists(CHECKPOINT_PATH):
            ...
            mlflow.pytorch.log_model(model, "image_clip_senet_model")
