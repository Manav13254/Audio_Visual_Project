...
FUSION_CHECKPOINT_PATH = "best_multimodal_weighted_fusion.pt"
    ...
    def __init__(self, clip_model, num_classes, checkpoint_path):
        ...
        try:
            self.full_model.load_state_dict(torch.load(checkpoint_path, map_location="cpu"))
             ...
             self.full_model.load_state_dict(torch.load(checkpoint_path, map_location="cpu", weights_only=False))

        # Feature extractor outputs 512-D vector (before the final classifier)
...
class AudioFeatureExtractor(nn.Module):
    def __init__(self, num_classes, checkpoint_path):
        ...
        try:
            ...
            backbone.load_state_dict(torch.load(checkpoint_path, map_location="cpu", weights_only=False))
...
class Gate(nn.Module):
    """
    A small MLP that predicts the scalar weight (alpha) for the image feature.
    Input: Concatenation of image and audio features (1024-D).
    Output: A single scalar alpha in [0, 1].
    """
    ...
    def forward(self, combined_features):
        alpha = self.gating_mlp(combined_features)
...
class WeightedFusionNet(nn.Module):
    def __init__(self, image_extractor, audio_extractor, num_classes, dropout_rate=0.5):
        super().__init__()
        self.image_backbone = image_extractor
        self.audio_backbone = audio_extractor
        ...
        feature_dim = image_extractor.output_dim 
        fusion_input_dim = feature_dim * 2       

        self.gate = Gate(fusion_input_dim, dropout_rate=dropout_rate)
    ...
    def forward(self, image, audio):
        ...
        combined_input = torch.cat([image_features, audio_features], dim=1) 
        alpha = self.gate(combined_input)
        alpha = alpha.unsqueeze(1) 
        
        final_feature = (alpha * image_features) + ((1.0 - alpha) * audio_features) # (B, 512)
        
        return self.fusion_head(final_feature)
...
if __name__ == '__main__':
    ...
    model = WeightedFusionNet(image_feature_extractor, audio_feature_extractor, NUM_CLASSES, dropout_rate=DROPOUT).to(DEVICE)
    
    trainable_params = list(model.gate.parameters()) + list(model.fusion_head.parameters())
